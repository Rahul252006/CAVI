...
export async function updateBrainConfig(companyId: string, updates: Partial<BrainConfig>) {
  ...
  const updated: BrainConfig = {
    ...
    primaryLanguage: updates.primaryLanguage || existing?.primaryLanguage || 'undetermined',
    allowCodeSwitching: updates.allowCodeSwitching ?? existing?.allowCodeSwitching ?? true,
    allowedActions: updates.allowedActions || existing?.allowedActions || ['create_ticket', 'escalate_to_human'],
    maxRefundAmount: updates.maxRefundAmount ?? existing?.maxRefundAmount ?? 0,
